...
def get_photos(dtime):
    ctime = datetime.datetime.now()
    ptime = ctime - datetime.timedelta(days = dtime)
    # return db.photos.find({'$and':[{'date':{'$gte':ptime}},{'date':{'$lte':ctime}}]}).sort("date",pymongo.DESCENDING)
    return db.photos.find({}).sort("date",pymongo.DESCENDING)
